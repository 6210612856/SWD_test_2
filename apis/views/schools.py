...
class StudentSubjectsScoreAPIView(APIView):
    ...
    def post(request, *args, **kwargs):
        ...
        if student_first_name is None or student_last_name is None:
            return Response(status=status.HTTP_400_BAD_REQUEST)

        StudentSubjectsScore.objects.filter(student__first_name=student_first_name, student__last_name=student_last_name,
                                            subjects__title=subjects_title).update_or_create(score=score)
...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        current_student = StudentSubjectsScore.objects.filter(Personnel.objects.filter(id=student_id))
        sum_grade = 0
        sum_credit = 0
        example_context_data = {}
        subjectall = []
        ...
        for subject in current_student:
            sum_credit += subject.credit
            sum_grade += subject.score * subject.credit
            if subject.score >= 80:
                grade = "A"
            elif subject.score >= 75:
                grade = "B+"
            elif subject.score >= 70:
                grade = "B"
            elif subject.score >= 65:
                grade = "C+"
            elif subject.score >= 60:
                grade = "C"
            elif subject.score >= 55:
                grade = "D+"
            elif subject.score >= 50:
                grade = "D"
            else:
                grade = "F"
            subject_detail = {
            ...
            subjectall.append(subject_detail)
        ...
        example_context_data["student"]["full_name"] = current_student[0].student.first_name + " " + current_student[0].student.last_name
        example_context_data["student"]["school"] = current_student[0].student.school.title
        example_context_data["subject_detail"] = subjectall
        example_context_data["grade_point_average"] = avg
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        your_result = []
#--------------------------------------------------------------------------------------------------------/
        personals = Personnel.objects.all()
        this_school = Classes.objects.filter(school=Schools.objects.filter(title=school_title)).values_list("class_order", flat=True).order_by("class_order")
        i = 1

        for x in this_school:
            person = Personnel.objects.filter(school_class=x)[0]         
            your_result.append(f"{i}. school: {person.school_title}, role: {person.personnel_type}, class: {x},name: {person.first_name} {person.last_name}")
...
class SchoolHierarchyAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        school_title = request.GET.get('name', None)

        school_name = Schools.objects.filter(title=school_title).first()
...
#--------------------------------------------------------------------------------------------------------------/
        ...
        your_result = Classes.objects.all().filter(school = Schools.objects.all().filter(title = name_)[0])

        if len(your_result) < 1:
